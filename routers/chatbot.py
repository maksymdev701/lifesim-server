...
async def create_chatbot(scenario: str = Body(...), image: UploadFile = File(...), role_play_system_prompt: str = Body(...), guide_system_prompt: str = Body(...), person_details: str = Body(...), person_voices: str = Body(...)):
    ...
    if is_existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="There is exising scenario about this kind of chatbot.")
    ...
    Chatbots.insert_one({"scenario": scenario, "image": generated_name, "role_play_system_prompt": role_play_system_prompt, "guide_system_prompt": guide_system_prompt, "person_details": list_person_details, "person_voices": list_person_voices, "slug": slug})
